...
Head over to the active cases per million indicator. It is very likely that all countries will show "no data" on the current date (default date when launching the app). This is normal because values for this this indicator are parsed off of JHU data sets, which are based on data available the day before. Use the slider to change the date and go back one day; for example, if today's date is 2020-09-26, change the date to 2020-09-25. Now most countries on the list should show numerical values instead of "no data", and country names are listed in descending order of these values. Select a few items at the top of the list; examine the graph and make a note of the countries with recent spikes in active cases per million. Without changing the item selection, switch to the positive rate indicator. Among the countries on the updated graph, make a note of those that also show recent increases, however small but above 2%, in the positive rate.
